...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        day_from = df['date_from'].dt.dayofweek
        day_to = df['date_to'].dt.dayofweek
        df['weekend_stay'] = (
                ((4 <= day_from) & (day_from < 6)) | ((4 < day_to) & (day_to < 6)) | (
                (day_to < day_from) & (day_from < 6)) |
                ((df['date_to'] - df['date_from']).dt.days >= 6)).astype("string")
    ...
    def add_night_price(df):
        ...
        # Write your code here #
        ########################
        df['night_price'] = np.round(df['accommodation_price'] / df['length_of_stay'] / df['n_rooms'], decimals=2)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        sum_columns = group_reservations.iloc[:, [1, 8, 9, 10, 11, 13, 14, 15, 16, 28]].groupby('group_id').sum()
        mean_columns = group_reservations.iloc[:, [1, 12]].groupby('group_id').mean()
        mode_columns = group_reservations.iloc[:, [1, 2, 3, 4, 5, 6, 17, 25, 26, 27]].groupby('group_id').agg(
            lambda x: x.value_counts().index[0])
        first_columns = group_reservations.iloc[:, [1, 24, 18, 19, 20, 21, 22]].groupby('group_id').first()

        group_reservations_agg_part_1 = pd.merge(sum_columns, mean_columns, on="group_id")
        group_reservations_agg_part_2 = pd.merge(mode_columns, first_columns, on="group_id")
        group_reservations = pd.merge(group_reservations_agg_part_1, group_reservations_agg_part_2, on="group_id")
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        ########################
        room_segment = []
        avg_night_price = df.groupby('room_group_id')['night_price'].mean().reset_index()
        values = avg_night_price['night_price'].tolist()

        for v in values:
            room_segment.append(self.map_value_to_bucket(v, self.room_segment_buckets))

        segment_mapping = {values[i]: room_segment[i] for i in range(len(room_segment))}
        avg_night_price['night_price'] = avg_night_price['night_price'].map(segment_mapping)
        final_mapping = dict(zip(avg_night_price['room_group_id'], avg_night_price['night_price']))
        df['room_segment'] = df['room_group_id'].map(final_mapping)
